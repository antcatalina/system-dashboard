...
const PLACEHOLDER = {
  cpu: { model: 'Waiting for data...', cores: 0, threads: 0, load: 0, temperature: 0, frequency: 0, maxFrequency: 1, perCore: [] },
  gpu: { model: 'Waiting for data...', utilization: 0, memoryUsed: 0, memoryTotal: 1, temperature: 0, fanSpeed: 0, powerDraw: 0, powerLimit: 1, coreClock: 0, memoryClock: 0, driverVersion: '---' },
  ram: { total: 1, used: 0, free: 1, usedPercent: 0, swapTotal: 0, swapUsed: 0 },
  monitors: [],
  network: { downloadSpeed: 0, uploadSpeed: 0, downloadTotal: 0, uploadTotal: 0, latency: 0, adapters: [], primaryAdapter: '' },
  fps: null,
};
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-surface flex flex-col">
      ...
      {/* Zero-gap dashboard — cards flush against each other and viewport edges */}
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <div>
          <NetworkCard
            ...
            history={history.map((h) => ({ download: h.networkDownload, upload: h.networkUpload }))}
          />
        </div>
      </main>
    </div>
